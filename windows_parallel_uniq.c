...
thrdFuncInfo initInfo(int from, int to, int readLines, char** buffer)
{
    ...
    {
        strcpy_s(info.part[currLine], MAX_LINE_LEN, buffer[i]);
...
DWORD WINAPI threadFindUnique(void* args)
{
    ...
    for (int i = 0; i < info->partSize; i++)
    {
        ...
        if (foundStrings < 1)
        {
            strcpy_s(info->result[results], MAX_LINE_LEN, info->part[i]);
...
// Runs threads via winAPI
int runThreadsWindows(thrdFuncInfo * partsInfo, HANDLE * threadArr, DWORD * threadId, int numOfThreads)
{
    // Create threads and find unique strings in each part
    for (int i = 0; i < numOfThreads; i++)
    {
        if ((threadArr[i] = CreateThread(NULL, 0, threadFindUnique, &partsInfo[i], 0, &threadId[i])) == NULL)
        {
    ...
    for (int i = 0; i < numOfThreads; i++)
        WaitForSingleObject(threadArr[i], INFINITE);
...
int main(int argc, char** argv)
{
    ...
    HANDLE * threadArr = (HANDLE *)malloc(numOfThreads * sizeof(HANDLE));
    DWORD * threadId = (DWORD*)malloc(numOfThreads * sizeof(DWORD));
    ...
    if(runThreadsWindows(partsInfo, threadArr, threadId, numOfThreads) == 1)
        return 1; 
